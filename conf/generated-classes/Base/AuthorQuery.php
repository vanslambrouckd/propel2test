 ...
 * @method     ChildAuthorQuery orderByBirtdate($order = Criteria::ASC) Order by the birtdate column
 *
 ...
 * @method     ChildAuthorQuery groupByBirtdate() Group by the birtdate column
 *
 ...
 * @method     ChildAuthor findOneByBirtdate(string $birtdate) Return the first ChildAuthor filtered by the birtdate column *
 ...
 * @method     ChildAuthor requireOneByBirtdate(string $birtdate) Return the first ChildAuthor filtered by the birtdate column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildAuthor[]|ObjectCollection findByBirtdate(string $birtdate) Return ChildAuthor objects filtered by the birtdate column
...
abstract class AuthorQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = '\\Author', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, firstname, lastname, email, birtdate FROM author WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the birtdate column
     *
     * Example usage:
     * <code>
     * $query->filterByBirtdate('2011-03-14'); // WHERE birtdate = '2011-03-14'
     * $query->filterByBirtdate('now'); // WHERE birtdate = '2011-03-14'
     * $query->filterByBirtdate(array('max' => 'yesterday')); // WHERE birtdate > '2011-03-13'
     * </code>
     *
     * @param     mixed $birtdate The value to use as filter.
     ...
     * @return $this|ChildAuthorQuery The current query, for fluid interface
     */
    public function filterByBirtdate($birtdate = null, $comparison = null)
    {
        if (is_array($birtdate)) {
            $useMinMax = false;
            if (isset($birtdate['min'])) {
                $this->addUsingAlias(AuthorTableMap::COL_BIRTDATE, $birtdate['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($birtdate['max'])) {
                $this->addUsingAlias(AuthorTableMap::COL_BIRTDATE, $birtdate['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AuthorTableMap::COL_BIRTDATE, $birtdate, $comparison);
    }
